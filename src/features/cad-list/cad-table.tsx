    ...
    <table className="w-full whitespace-nowrap text-left text-sm" aria-label="등록된 도면">
      ...
      <tbody className="divide-y divide-slate-100">
        {items.map(file => <tr key={file.id} id={`version-${file.id}`} className="hover:bg-slate-50/80 target:bg-teal-50">
          ...
          <td className="px-4 py-4"><div className="flex items-start gap-3">{<Link href={`/cad/versions/${file.id}/viewer`} className="pt-2 text-xs font-medium text-teal-700">도면 보기</Link>}<CopyCadLink versionId={file.id} format={file.fileFormat as 'DXF' | 'DWG'} label={`${file.displayName} 도면 링크 복사`}/>{manageCurrent ? !file.isCurrent && <span className="pt-1"><CurrentButton locationId={file.locationId} versionId={file.id}/></span> : <Link href={`/cad/locations/${file.locationId}#version-${file.id}`} className="pt-2 text-xs font-medium text-teal-700 hover:underline">버전 정보</Link>}<a href={`/api/cad-files/${file.id}/content`} className="pt-2 text-slate-400 hover:text-teal-700" aria-label={`${file.originalFilename} 원본 다운로드`}><Download size={17}/></a><DeleteButton versionId={file.id} filename={file.displayName} version={file.version} location={`${file.businessUnit} / ${file.site} / ${file.building} / ${file.floor}`} isCurrent={file.isCurrent}/></div></td>
        </tr>)}
      </tbody>
    </table>
  </div>;
