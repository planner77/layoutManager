...
export type Registration = LocationInput & { registeredAt: string; makeCurrent: boolean; description: string; deletePassword: string };
...
export function registration(input: Record<string, unknown>): Registration {
  ...
  return { ...result, registeredAt: date, makeCurrent: current === true || current === 'true', description: cadDescription(input.description), deletePassword: validateDeletePassword(input.deletePassword) };
}
