...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <section>
    ...
    <div className="mb-3 flex items-center justify-between text-sm"><p className="text-slate-500">{filtered ? '검색 결과' : '등록된 도면'} <strong className="ml-1 text-slate-800">{result.total.toLocaleString('ko-KR')}개</strong></p><p className="text-xs text-slate-400">등록일 최신순 · {versionScope}</p></div>
