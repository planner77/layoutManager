...
export default defineConfig({
  ...
  webServer: { command: 'npm run db:deploy && npm run start -- --port 3101', url: 'http://127.0.0.1:3101', reuseExistingServer: false, env: { CAD_STORAGE_BACKEND: process.env.CAD_E2E_STORAGE_BACKEND === 's3' ? 's3' : 'local', DATABASE_URL: `file:${data}/db.sqlite`, CAD_STORAGE_PATH: `${data}/cad`, MAX_UPLOAD_SIZE_MB: '1' } },
});
