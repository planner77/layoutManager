...
async function upload(request: APIRequestContext, name: string, makeCurrent = false) {
  ...
  return response.json() as Promise<{id:string;locationId:string;displayName:string}>;
}
...
test('TC-LINK-001/002: list, location and viewer create pinned renderer-specific links', async ({ page, request, browser, baseURL }) => {
  ...
  await expect(page.getByRole('row').filter({hasText:'link-v1.dxf'}).getByRole('button', {name:/도면 링크 복사$/})).toBeVisible();
  ...
  await expect(direct.getByRole('heading', {name:first.displayName})).toBeVisible();
