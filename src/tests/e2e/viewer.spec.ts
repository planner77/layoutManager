...
for (const renderer of ['dxf-viewer','three-dxf-viewer']) test(`TC-DXF/THREE-001: ${renderer} render, controls, resize and re-entry`,async({page,request})=>{
  ...
  await expect.poll(async()=>canvas.evaluate(c=>{
    const rect=c.getBoundingClientRect(),element=c as HTMLCanvasElement;
    return Math.abs(element.width-rect.width*devicePixelRatio)<2 && Math.abs(element.height-rect.height*devicePixelRatio)<2;
  })).toBe(true);
  ...
  await page.goto(`/cad/versions/${file.id}/viewer?renderer=${renderer}`);await expect(page.getByRole('status')).toHaveText('도면 표시 완료');
  await page.screenshot({path:`test-results/${renderer}-render.png`,fullPage:true});expect(errors).toEqual([]);
});
