...
기준일: 2026-09-07. `요구사항 확정`은 사용자 원문이 결정한 사항, `제안`은 이 계획 확인 후 채택할 기본안, `확인 대기`는 별도 중요한 판단이 필요한 사항이다. 현재 전체 구현 계획 확인은 받지 않았다. 작성된 문서는 구현 승인을 의미하지 않는다.
...
- Consequences: Unit 7A를 0B 직후 최소 DWG parse/render/free 실험으로 수행한다. 초기 최소 기하 성공은 최종 업무 도면 호환성 증명이 아니다. 구현 전 사용자에게 이 구체적인 안을 확인한다.
...
| 항목 | 상태 | 다음 조치 |
| --- | --- | --- |
| 전체 구현 계획 | 사용자 확인 대기 | 이 문서/Architecture/TestPlan 기준 확인 후 0B 시작 |
| ADR-004 직접 DWG 렌더링 | 사용자 확인 대기 | Three.js를 그리기 기반으로 사용하는 안 확인 |
